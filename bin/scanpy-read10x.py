...
if __name__ == "__main__":
    ...
    adata = sc.read(args.data_dir + 'matrix.mtx', cache=True).T  # transpose the data
    genes = pd.read_csv(args.data_dir + 'genes.tsv', header=None, sep='\t')
    adata.var_names = genes[1]
    adata.var['gene_ids'] = genes[0]  # add the gene ids as annotation of the variables/genes
    adata.obs_names = pd.read_csv(args.data_dir + 'barcodes.tsv', header=None)[0]
